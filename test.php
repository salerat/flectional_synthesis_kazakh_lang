...
array('rus' => ' – наша подушка ', 'type' => ';possessive_p_1;', 'wordTransform' => 'жастығымыз', 'word' => 'жасты'),
...
array('rus' => ' – ваша встреча', 'type' => ';possessive_s_2_r;',
 'wordTransform' => 'кездесуiңiз', 'word' => 'кездесу'),
);

$testArray = $testFacesArr;
...
echo '<td>' . $faceArr['wordTransform'] . '</td>
 <td>' . $faceArr['rus'] . '</td>';
if( !empty($resultArray)) {
 if ($resultArray[$faceArr['type']]['word'] === $faceArr['wordTransform']) {
 echo '<td>Верно</td>';
 } else {
 echo '<td>Неверно</td>';
 }
